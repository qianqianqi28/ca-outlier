# This is for CA: Figure 1 (b) in the Section Empirical studies/Results
rm(list=ls()) 
library(readxl)
library(xtable)
library(FactoMineR)
...
#get the value for outlying cells; the value is 27.00926
X[which(rownames(dt) == "Volvo"), which(colnames(dt) == "Safety")]
...
recon[which(rownames(dt) == "Volvo"), which(colnames(dt) == "Safety")]
(recon^2)[which(rownames(dt) == "Volvo"), which(colnames(dt) == "Safety")]
...
#get the value for outlying cells; the value is 13.0753
X[which(rownames(dt) == "Volvo"), which(colnames(dt) == "Safety")]
